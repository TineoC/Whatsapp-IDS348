...
function Sidebar({ chats }) {
	...
	const handleContactInfo = () => {
		if (inputEmailContact !== location.state) {
			...
			alert(
				"No puede ingresarse a sí mismo como un usuario, favor ingrese un correo electrónico distinto al suyo"
			);
		}
	};
	...
	const handleCreateChat = () => {
		if (users.length === 0) {
			alert(
				"Error: No ha seleccionado ningún usuario, favor elegir el(los) usuarios que desee"
			);
		...
		<div className='sidebar'>
						...
						>
							<form>
								<input
									...
									onChange={(e) =>
										setInputEmailContact(e.target.value)
									}
			...
			<div className='sidebar_newchatContainer'>
				...
				<div className='sidebar_modal'>
					...
					<Modal
						title='Enter Group Chat Information'
						onClose={() => setShow2(false)}
						show={show2}
						onSave={() =>
							handlechatInfo(chatUsers, inputName, inputPicture)
						}
					>
						<form>
							...
							<input
								...
								value={inputPicture}
								onChange={(e) =>
									setInputPicture(e.target.value)
								}
							/>
						</form>
					</Modal>
			...
			<div className='sidebar_search'>
				<div className='sidebar_searchContainer'>
					...
					<input
						...
						value={input}
					/>
					<SearchButton onClick={() => searchChat()} type='sumbit'>
						Search
					</SearchButton>
				</div>
			</div>
